...
//   sendCorrection(code, playerId, balls, nextTurn, extra)
...
export async function joinRoom(codeRaw) {
  ...
  await update(ref(db, `rooms/${code}/players`), { [slot]: { name } });
...
export function sendCorrection(code, playerId, balls, nextTurn, extra = {}) {
  ...
  if (extra.newScore !== undefined) updates[`scores/${playerId}`] = extra.newScore;
  if (extra.groups !== undefined) updates.groups = extra.groups;
  if (extra.gameOver !== undefined) updates.gameOver = extra.gameOver;
  if (extra.foul !== undefined) updates.foul = extra.foul;
